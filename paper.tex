...
A view of a specific resource with details such as definition, chapter, alternative labels and textbook page numbers is presented using an installation of the LodView RDF browser~\footnote{\url{https://lodview.it/}} with a modified color scheme according to the SNIK subontology color coding.
...
UC6
The visualization of the instances belonging to a specific class.
The instances in SNIK Graph are shown near their specific class(es). \todo{Konrad: stimmt das so? hier war ich mir sehr unsicher}

UC7
The visualization of the paths that link different instances.
...
A fulltext search helps users to find their start ressource, path and expand operations leads them to an interesting exploration.

UC13-15
Filtering.
SNIK Grah provides several filter options.
Users can decide which sources (textbook) to use, what kinds of types depending on our meta model do display and can also filter along relations.
