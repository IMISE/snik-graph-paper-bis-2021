...
\todo{Vorschlag von Michelle: Verwendung in der Lehre setzt intuitive/leicht erlernbare Benutzung des Graphen voraus um zu vermeiden wertvolle Vorlesungszeit an ein Tutorial zu verlieren --> Bekannet Tastaturshortcuts, Drag&Drop, übersichtliches }
...
\begin{figure}[h!]
    ...
    \caption{Visual search results for \enquote{Chief Information Officer} in SNIK Graph. Automatically generated interlinks that are not yet manually validated are shown with an exclamation mark.}\label{fig:search}
\end{figure}
...
Search results can then be selected in the graph, see~\cref{fig:search}, which allows chaining the path and neighbourhood operations described in their respective sections.%cannot cref because of unnumbered section star commands
...
% Research into most interesting short paths between objects relfinder, wisp, anna brakemeier, anatoli zeiser
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\linewidth]{spiderworm.pdf}
    \caption{\emph{Spiderworm} between \aurl{bb}{PatientIdentificationNumber} and \aurl{bb}{QualityOfData} in SNIK Graph.}
	\label{fig:path}
\end{figure}
\vspace{-3pt}
